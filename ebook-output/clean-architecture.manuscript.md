...
##### 1.1.4.2.3 **プレゼンテーション層** - Controller
...
flowchart TD
    ...
    Title --> A["1️⃣ 独立性 (Independence)<br/>ビジネスロジックがフレームワークに依存しない"]
    Title --> B["2️⃣ テスト性 (Testability)<br/>外部ツールなしにテストできる"]
    Title --> C["3️⃣ 保守性 (Maintainability)<br/>変更が局所的で、相互影響が少ない"]
```
...
```mermaid
flowchart TD
    A["独立性<br/>ビジネスロジックがフレームワークに依存しない<br/>(独立性が高い)"] --> B["テスト性<br/>外部ツールなしにテストできる<br/>(テストしやすい)"]
    B --> C["安全に修正・拡張できる"]
    C --> D["保守性<br/>変更が局所的で、相互影響が少ない<br/>(保守性が高い)"]
    D -.-> A
```
...
## 2.1 01. 単一責任の原則 (SRP) - Single Responsibility Principle
...
## 2.2 02. 開放閉鎖の原則 (OCP) - Open/Closed Principle
...
## 2.3 03. リスコフの置換原則 (LSP) - Liskov Substitution Principle
...
## 2.4 04. インターフェース分離の原則 (ISP) - Interface Segregation Principle
...
## 2.5 05. 依存性逆転の原則 (DIP) - Dependency Inversion Principle
...
## 3.1 01. プレゼンテーション層 (Presentation Layer)
...
## 3.2 02. アプリケーション層 (Application Layer)
...
## 3.4 04. インフラストラクチャ層 (Infrastructure Layer)
...
## 3.5 05. 層の依存関係ルール (Layer Dependency Rules)
...
#### 3.5.2.1 違反1：ドメイン層が外部フレームワークに依存
...
#### 3.5.2.3 違反3：アプリケーション層がプレゼンテーション層に依存
...
## 4.1 01. 依存性注入 (Dependency Injection)
...
## 4.2 02. リポジトリパターン (Repository Pattern)
...
## 4.3 03. サービスパターン (Service Pattern)
...
```
✅ ドメインサービスはインターフェースのみに依存（DBやフレームワークを知らない）
✅ 複数のエンティティにまたがるビジネスルールがドメインサービスに集約されている
✅ アプリケーションサービス（ユースケース）はプロセスの手順（取得・呼び出し・永続化・通知）に専念している
...
## 4.4 04. DTO パターン (Data Transfer Object)
...
## 4.5 05. アダプタパターン (Adapter Pattern)
...
## 7.5 04: 循環依存（Circular Dependency）
...
## 8.3 02: SNS プラットフォーム（フィード生成・キャッシング）
