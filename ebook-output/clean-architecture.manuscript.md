...
# 第1章 Introduction {#chapter-01-introduction}

## 1.1 クリーンアーキテクチャ概要 {#section-01-introduction-01-overview}
...
## 1.2 なぜクリーンアーキテクチャが必要か？ {#section-01-introduction-02-why-clean-architecture}
...
## 1.3 クリーンアーキテクチャの3つの主要概念 {#section-01-introduction-03-key-concepts}
...
# 第2章 Core Principles {#chapter-02-core-principles}

## 2.1 単一責任の原則 (SRP) - Single Responsibility Principle {#section-02-core-principles-01-single-responsibility}
...
## 2.2 開放閉鎖の原則 (OCP) - Open/Closed Principle {#section-02-core-principles-02-open-closed}
...
## 2.3 リスコフの置換原則 (LSP) - Liskov Substitution Principle {#section-02-core-principles-03-liskov-substitution}
...
## 2.4 インターフェース分離の原則 (ISP) - Interface Segregation Principle {#section-02-core-principles-04-interface-segregation}
...
## 2.5 依存性逆転の原則 (DIP) - Dependency Inversion Principle {#section-02-core-principles-05-dependency-inversion}
...
# 第3章 Architecture Layers {#chapter-03-architecture-layers}

## 3.1 プレゼンテーション層 (Presentation Layer) {#section-03-architecture-layers-01-presentation-layer}
...
## 3.2 アプリケーション層 (Application Layer) {#section-03-architecture-layers-02-application-layer}
...
## 3.3 ドメイン層 (Domain Layer) {#section-03-architecture-layers-03-domain-layer}
...
## 3.4 インフラストラクチャ層 (Infrastructure Layer) {#section-03-architecture-layers-04-infrastructure-layer}
...
## 3.5 層の依存関係ルール (Layer Dependency Rules) {#section-03-architecture-layers-05-layer-dependencies}
...
# 第4章 Design Patterns {#chapter-04-design-patterns}

## 4.1 依存性注入 (Dependency Injection) {#section-04-design-patterns-01-dependency-injection}
...
## 4.2 リポジトリパターン (Repository Pattern) {#section-04-design-patterns-02-repository-pattern}
...
## 4.3 サービスパターン (Service Pattern) {#section-04-design-patterns-03-service-pattern}
...
## 4.4 DTO パターン (Data Transfer Object) {#section-04-design-patterns-04-dto-pattern}
...
## 4.5 アダプタパターン (Adapter Pattern) {#section-04-design-patterns-05-adapter-pattern}
...
# 第5章 実装ガイド - 完全マニュアル {#chapter-05-implementation-guide}

## 5.1 プロジェクト構造 {#section-05-implementation-guide-01-project-structure}
...
#### 5.1.2.2 2️⃣ Application（アプリケーション層）
...
#### 5.1.2.3 3️⃣ Presentation（プレゼンテーション層）
...
#### 5.1.2.4 4️⃣ Infrastructure（インフラ層）
...
## 5.2 エンティティ設計 {#section-05-implementation-guide-02-entity-design}
...
### 5.2.4 📝 実装例 1: Email 値オブジェクト

#### 5.2.4.1 ❌ 悪い例（値オブジェクトがない）
...
## 5.3 ユースケース設計 {#section-05-implementation-guide-03-usecase-design}
...
#### 5.3.4.1 1️⃣ ユースケース定義（リクエスト/レスポンス）
...
#### 5.3.4.2 2️⃣ インターフェース定義（依存性の抽象化）
...
### 5.3.7 🌐 Controller から ユースケース呼び出し
...
## 5.4 完全な実装例 {#section-05-implementation-guide-04-implementation-example}
...
### 5.4.2 💾 01: Domain 層の実装

#### 5.4.2.1 domain/errors/DomainError.ts
...
#### 5.4.2.2 domain/value-objects/Email.ts
...
#### 5.4.2.3 domain/value-objects/Password.ts
...
#### 5.4.2.4 domain/entities/User.ts
...
#### 5.4.2.5 domain/interfaces/IUserRepository.ts
...
### 5.4.3 🔄 02: Application 層の実装

#### 5.4.3.1 application/errors/ApplicationError.ts
...
#### 5.4.3.2 application/dtos/RegisterUserRequest.ts
...
#### 5.4.3.3 application/dtos/LoginUserRequest.ts
...
#### 5.4.3.4 application/interfaces/
...
#### 5.4.3.5 application/usecases/RegisterUserUseCase.ts
...
#### 5.4.3.6 application/usecases/LoginUserUseCase.ts
...
### 5.4.4 🌐 03: Presentation 層の実装

#### 5.4.4.1 presentation/controllers/UserController.ts
...
#### 5.4.4.2 presentation/routes/userRoutes.ts
...
### 5.4.5 💾 04: Infrastructure 層の実装

#### 5.4.5.1 infrastructure/database/MySQLConnection.ts
...
#### 5.4.5.2 infrastructure/repositories/UserRepository.ts
...
#### 5.4.5.3 infrastructure/services/BcryptHasher.ts
...
#### 5.4.5.4 infrastructure/services/EmailAdapter.ts
...
#### 5.4.5.5 infrastructure/services/JwtTokenGenerator.ts
...
### 5.4.6 ⚙️ 05: Config 層（DI 設定）

#### 5.4.6.1 config/Container.ts
...
## 5.5 テスト戦略 {#section-05-implementation-guide-05-testing-strategy}
...
### 5.5.2 🧪 01: Domain 層テスト（ユニットテスト）

#### 5.5.2.1 Domain テストの特徴

- **外部依存なし** - DB、API 呼び出しなし
- **最も高速** - 数ミリ秒で実行
- **本数が多い** - ビジネスロジックをすべてカバー

#### 5.5.2.2 Email 値オブジェクトテスト
...
### 5.5.3 📋 02: Application 層テスト

#### 5.5.3.1 Application テストの特徴

- **モックが必要** - Repository、ExternalService をモック
- **ユースケース全体をテスト** - エンドツーエンドロジック
- **テストダブル** - Mock / Stub

#### 5.5.3.2 RegisterUserUseCase テスト
...
### 5.5.5 🧪 04: Controller（Presentation）テスト
...
# 第6章 ベストプラクティス - 実装品質の向上 {#chapter-06-best-practices}

## 6.1 命名規則 {#section-06-best-practices-01-naming-conventions}
...
## 6.2 エラーハンドリング {#section-06-best-practices-02-error-handling}
...
#### 6.2.4.2 パターン2: Result オブジェクトで結果を返す
...
## 6.3 ロギング・監視 {#section-06-best-practices-03-logging-monitoring}
...
#### 6.3.3.1 プレゼンテーション層：リクエスト/レスポンス
...
## 6.4 パフォーマンス最適化 {#section-06-best-practices-04-performance-optimization}
...
## 6.5 セキュリティ {#section-06-best-practices-05-security}
...
### 6.5.3 🔑 Authentication/Authorization
...
#### 6.5.3.2 Role-Based Access Control（RBAC）
...
# 第7章 よくある間違い - アンチパターンの認識と回避 {#chapter-07-common-pitfalls}

## 7.1 過度な設計（Over-Engineering） {#section-07-common-pitfalls-01-over-engineering}
...
## 7.2 密結合（Tight Coupling） {#section-07-common-pitfalls-02-tight-coupling}
...
### 7.2.4 🔄 よくある密結合パターンと修正

#### 7.2.4.1 パターン1：グローバルゴブリン
...
## 7.3 貧血モデル（Anemic Model） {#section-07-common-pitfalls-03-anemic-model}
...
## 7.4 循環依存（Circular Dependency） {#section-07-common-pitfalls-04-circular-dependency}
...
#### 7.4.6.3 Webpack/TypeScript での検出
...
#### 7.4.6.4 TypeScript での検出（tsc --diagnostics）
...
# 第8章 ケーススタディ - 実世界の実装例 {#chapter-08-case-studies}

## 8.1 ECサイト（注文・決済・在庫管理） {#section-08-case-studies-01-ecommerce-site}
...
## 8.2 SNS プラットフォーム（フィード生成・キャッシング） {#section-08-case-studies-02-sns-platform}
...
## 8.3 マイクロサービス（サービス分割・イベント駆動） {#section-08-case-studies-03-microservices}
...
#### 8.3.3.3 Inventory Service：イベントリスナー
...
#### 8.3.3.4 Payment Service：イベントリスナー
...
# 第9章 ツール・リソース - 実装支援 {#chapter-09-tools-and-resources}

## 9.1 フレームワーク比較 {#section-09-tools-and-resources-01-frameworks}
...
### 9.1.2 🔷 Node.js / TypeScript フレームワーク

#### 9.1.2.1 ① Express.js + Type-DI
...
### 9.1.4 🐍 Python フレームワーク

#### 9.1.4.1 ① FastAPI + Dependency Injector
...
## 9.2 DI コンテナー・IoC フレームワーク {#section-09-tools-and-resources-02-di-containers}
...
### 9.2.2 🔷 TypeScript 推奨 3選

#### 9.2.2.1 ① Type-DI（最もシンプル）
...
#### 9.2.3.1 ① Spring DI（Spring Boot 組み込み）
...
## 9.3 開発ツール（テスト・分析・ビルド） {#section-09-tools-and-resources-03-development-tools}
...
#### 9.3.1.1 ユニットテスト：Jest（Node.js 標準）
...
#### 9.3.1.2 統合テスト：Supertest（HTTP テスト）
...
#### 9.3.1.3 E2E テスト：TestContainers（本物の DB でテスト）
...
#### 9.3.2.1 静的解析：ESLint + TypeScript Plugin
...
## 9.4 学習リソース {#section-09-tools-and-resources-04-learning-resources}
...
#### 9.4.1.1 第1順位：「Clean Architecture」 Robert C. Martin
...
#### 9.4.1.2 第2順位：「Domain-Driven Design」Eric Evans
...
#### 9.4.1.3 第3順位：「Building Microservices」Sam Newman
...
#### 9.4.1.4 番外編：「Refactoring」Martin Fowler
...
#### 9.4.4.1 GitHub リポジトリ

##### 9.4.4.1.1 ① clean-architecture-manga
...
##### 9.4.4.1.2 ② nestjs-clean-architecture-example
...
##### 9.4.4.1.3 ③ typescript-clean-architecture-examples
