...
```mermaid
flowchart TD
    C["外部フレームワーク・ツール（DB、Web、API）"]
    B["アプリケーションロジック（ユースケース）"]
    A["ビジネスロジック（ドメイン）"]
    C -->|依存する| B
    B -->|依存する| A
```
...
クリーンアーキテクチャの目的は、**業務ロジックを技術的な実装から守ること**です。この目的のために、次の3つの原則をこの順番で適用します。

1. **関心事を分離する** — 各層が何に責任を持つかを決める（例：ドメイン層＝業務ルール、インフラ層＝DB/API連携）
2. **境界を定義する** — 「What（何をしたいか）」と「How（どう実現するか）」の間に線を引く
3. **依存方向を制御する** — 境界をまたぐ矢印を、必ず内側（ドメイン層）に向ける

```mermaid
flowchart LR
    A["① 関心事を分離する<br/>(各層の責任を決める)"]
    B["② 境界を定義する<br/>(What / How の線引き)"]
    C["③ 依存方向を制御する<br/>(矢印を内側へ)"]
    A --> B
    B --> C
```

よくある誤解は、③（依存方向のルール）だけを「クリーンアーキテクチャの正体」と捉えてしまうことです。実際には③は①②を正しく行った**結果**であり、①②を飛ばして「内側に依存しなければOK」と機械的に守っても、層を分けている本来の意味（変更理由が異なるコードを混在させない）は得られません。

このガイド全体で、①②③がそれぞれどのセクションに対応するかは次の通りです。

| 原則 | 主に扱うセクション |
|---|---|
| ① 関心事の分離 | [03-architecture-layers/](../03-architecture-layers/)（各層の責務） |
| ② 境界の定義 | [03-architecture-layers/05-layer-dependencies.md](../03-architecture-layers/05-layer-dependencies.md) |
| ③ 依存方向の制御 | [02-core-principles/05-dependency-inversion.md](../02-core-principles/05-dependency-inversion.md)、[03-architecture-layers/05-layer-dependencies.md](../03-architecture-layers/05-layer-dependencies.md) |

---

### 1.1.4 📊 典型的なアーキテクチャ図

#### 1.1.4.1 クリーンアーキテクチャの4層構造

```mermaid
flowchart TD
    Pres["プレゼンテーション層<br/>(UI, Web Controller, API, CLI)"]
    App["アプリケーション層<br/>(ユースケース, サービス)"]
    Dom["ドメイン層<br/>(エンティティ, ビジネスロジック)"]
    Infra["インフラストラクチャ層<br/>(DB, 外部API, ファイルシステム)"]
    Pres -->|依存する| App
    App -->|依存する| Dom
    Infra -->|依存する（インターフェース経由）| Dom
```
...
#### 1.1.5.2 ✅ クリーンアーキテクチャ設計

```mermaid
flowchart TD
    A["プレゼンテーション層（外側）<br/>controller.ts<br/>← HTTPリクエストを受け取る"]
    B["アプリケーション層<br/>CreateUserUseCase.ts<br/>← ビジネスロジック実行"]
    C["ドメイン層（最も内側）<br/>User エンティティ ← ビジネスルール<br/>UserRepository インターフェース"]
    D["インフラストラクチャ層（外側・実装詳細）<br/>MySQLUserRepository.ts<br/>← 実装詳細"]
    A --> B
    B --> C
    C -.->|実行時にインターフェース経由で呼ばれる| D
```

##### 1.1.5.2.1 **ドメイン層** - ビジネスルール
...
##### 1.1.5.2.3 **プレゼンテーション層** - Controller
...
## 1.2 なぜクリーンアーキテクチャが必要か？

> **コンセプト**: ソフトウェアの複雑性が増すほど、設計の重要性は高まる。クリーンアーキテクチャは、その複雑性に対処するための実証済みの方法。

### 1.2.1 🚨 実際の問題：レガシーコードの悪循環

#### 1.2.1.1 シナリオ：スタートアップから成長段階へ

##### 1.2.1.1.1 **Phase 1: MVP（最小限の製品）- 最初は快適**
...
flowchart TD
    ...
    D --> E["全体で15ファイルを修正"]
    E --> F["どこか1つ修正漏れがあるとバグになる"]
```

---

### 1.2.3 ✅ クリーンアーキテクチャが解決すること

#### 1.2.3.1 解決1️⃣: 関心事の分離
...
#### 1.2.3.4 解決4️⃣: ビジネスロジックの集約

```mermaid
flowchart TD
    A["「ユーザー登録の条件を変更」"] --> B["クリーンアーキテクチャ:"]
    B --> C["User エンティティ（ドメイン層）の1ファイルを修正"]
    C --> D["それだけ（変更が集約されている）"]
```
    ...
    %%{
      init: {
        "themeVariables": {
          ...
          "xyChart": {
            "plotColorPalette": "#5470c6, #91cc75"
          }
        }
      }
    }%%
...
- 従来設計: 初期は速く見えても、成長すると速度が落ちやすい
- クリーンアーキテクチャ: 最初は少しだけ手間がかかるが、長期的には高速を維持しやすい
...
## 1.3 クリーンアーキテクチャの3つの主要概念

> **コンセプト**: クリーンアーキテクチャは3つの重要な特性で成り立っている：独立性、テスト性、保守性。

これら3つの特性は、[01-overview.md](./01-overview.md#-本質3つの原則)で説明した「①関心事の分離 → ②境界の定義 → ③依存方向の制御」という3原則を適用した**結果**として得られるものです。3原則が「どうやって設計するか」、この3特性は「その結果どんな性質が得られるか」を表します。

### 1.3.1 🎯 3つの主要概念

```mermaid
flowchart TD
    Title["クリーンアーキテクチャの3本柱"]
    A["1️⃣ 独立性 (Independence)<br/>ビジネスロジックがフレームワークに依存しない"]
    B["2️⃣ テスト性 (Testability)<br/>外部ツールなしにテストできる"]
    C["3️⃣ 保守性 (Maintainability)<br/>変更が局所的で、相互影響が少ない"]
    Title --> A
    Title --> B
    Title --> C
```
...
##### 1.3.4.3.1 **修正例1: メールバリデーション形式を変更**
...
##### 1.3.4.3.2 **修正例2: DBをPostgreSQLに変更**
...
### 1.3.5 🎓 まとめ：3つの概念の相互関係

```mermaid
flowchart TD
    A["独立性<br/>ビジネスロジックがフレームワークに依存しない<br/>(独立性が高い)"]
    B["テスト性<br/>外部ツールなしにテストできる<br/>(テストしやすい)"]
    C["安全に修正・拡張できる"]
    D["保守性<br/>変更が局所的で、相互影響が少ない<br/>(保守性が高い)"]
    A --> B
    B --> C
    C --> D
    D -.-> A
...
#### 2.2.3.3 Step 3: アプリケーション層で抽象化を使用
...
#### 2.2.5.3 パターン3: Template Method パターン
...
#### 2.4.2.1 シナリオ：複数機能を持つWorkerインターフェース
...
#### 2.4.3.1 Step 1: 責任ごとインターフェースを分割
...
#### 2.4.3.2 Step 2: インターフェースを組み合わせて実装
...
## 2.5 依存性逆転の原則 (DIP) - Dependency Inversion Principle

> **原則**: 高レベルモジュール（ビジネスロジック）は低レベルモジュール（実装詳細）に依存してはいけない。両方とも抽象化に依存すべき。さらに、抽象化は詳細に依存してはならず、詳細が抽象化に依存すべきである。
...
#### 2.5.3.1 Step 1: 抽象化（インターフェース）を定義
...
#### 2.5.3.2 Step 2: 高レベルモジュール（ビジネスロジック）
...
### 3.1.4 💻 実装例：ユーザー作成エンドポイント

#### 3.1.4.1 Step 1: リクエスト/レスポンスDTO
...
#### 3.1.4.3 Step 3: マッピング（ドメイン層 ↔ プレゼンテーション層）
...
❌ インフラストラクチャ層がしてはいけない：
  - ビジネスロジック
  - ユースケース管理
  - プレゼンテーションロジック（レスポンス整形など）
```

---

### 3.4.3 🏗️ 典型的なインフラストラクチャ層の構成
...
## 3.5 層の依存関係ルール (Layer Dependency Rules)

> **最も重要なルール**: 内側の層は外側の層に依存しない。外側の層が内側の層に依存する一方向のみ。

### 3.5.1 🧭 このページの前提：Clean Architectureの3原則
...
#### 3.5.1.1 ①②の適用：各層は「What」と「How」のどちらを担うか
...
#### 3.5.3.1 違反1：ドメイン層が外部フレームワークに依存
...
#### 3.5.3.3 違反3：アプリケーション層がプレゼンテーション層に依存
...
## 4.1 依存性注入 (Dependency Injection)
...
#### 4.1.2.1 パターン1：コンストラクタインジェクション（推奨）
...
#### 4.1.2.3 パターン3：インターフェースインジェクション
...
#### 4.1.3.2 InversifyJS を使った DI コンテナ
...
## 4.2 リポジトリパターン (Repository Pattern)
...
#### 4.2.2.1 Step 1: インターフェース定義（ドメイン層）
...
## 4.3 サービスパターン (Service Pattern)

> **パターン**: ビジネスロジックをサービスクラスで実装。ドメイン層とアプリケーション層で使用。

### 4.3.1 🎯 2種類のサービス

#### 4.3.1.1 ドメインサービス（ドメイン層）
...
#### 4.3.1.2 アプリケーションサービス（アプリケーション層）
...
### 4.3.2 📊 ドメインサービス vs アプリケーションサービス
...
```
✅ ドメインサービスはインターフェースのみに依存
   （DBやフレームワークを知らない）
✅ 複数のエンティティにまたがる
   ビジネスルールがドメインサービスに集約されている
✅ アプリケーションサービス（ユースケース）は
   プロセスの手順（取得・呼び出し・永続化・通知）
   に専念している
✅ ドメインサービスがテストで単体検証できる
```
...
## 4.4 DTO パターン (Data Transfer Object)
...
### 4.5.2 💻 実装例1：メールサービスの統一

#### 4.5.2.1 複数のメールプロバイダーがある
...
| ファイル | 内容 |
|---------|------|
| [02-project-structure.md](./02-project-structure.md) | プロジェクトフォルダ構成 |
| [03-entity-design.md](./03-entity-design.md) | エンティティ設計の実装 |
| [04-usecase-design.md](./04-usecase-design.md) | ユースケース設計と実装 |
| [05-implementation-example.md](./05-implementation-example.md) | 完全な実装例 |
| [06-testing-strategy.md](./06-testing-strategy.md) | テスト戦略 |
...
#### 5.1.3.1 Level 1: 基本構造の理解
[02-project-structure.md](./02-project-structure.md) で、プロジェクトをどのようにフォルダを整理するか学びます。

#### 5.1.3.2 Level 2: ドメインモデルの実装
[03-entity-design.md](./03-entity-design.md) で、エンティティと値オブジェクトの具体的な設計方法を学びます。

#### 5.1.3.3 Level 3: ユースケースの実装
[04-usecase-design.md](./04-usecase-design.md) で、ビジネスロジック実行層の設計と実装を学びます。

#### 5.1.3.4 Level 4: 統合実装例
[05-implementation-example.md](./05-implementation-example.md) で、全層を統合した完全な実装例を見ます。

#### 5.1.3.5 Level 5: テスト戦略
[06-testing-strategy.md](./06-testing-strategy.md) で、各層のテスト方法を学びます。
...
### 5.1.7 📌 各ファイルへのナビゲーション

**まず最初に読むべき:**
1. [プロジェクト構造](./02-project-structure.md) - 全体像を把握
2. [エンティティ設計](./03-entity-design.md) - ドメインモデル実装
3. [完全実装例](./05-implementation-example.md) - 動く全体例を見る

**次に深掘り:**
4. [ユースケース設計](./04-usecase-design.md) - アプリケーション層の詳細
5. [テスト戦略](./06-testing-strategy.md) - 品質確保の方法
...
#### 5.1.11.1 [02-project-structure.md](./02-project-structure.md)
- フォルダ構成図
- ファイル配置の意図
- 各層の役割・責務と依存関係のルール

#### 5.1.11.2 [03-entity-design.md](./03-entity-design.md)
- User エンティティの完全実装
- Email、Password 値オブジェクト
- バリデーション戦略

#### 5.1.11.3 [04-usecase-design.md](./04-usecase-design.md)
- RegisterUserUseCase 実装
- トランザクション管理
- エラーハンドリング

#### 5.1.11.4 [05-implementation-example.md](./05-implementation-example.md)
- 全層を含む完全例
- ユーザー登録・ログインの一連の流れを追いながら学ぶ
- 実際に動かせるコード

#### 5.1.11.5 [06-testing-strategy.md](./06-testing-strategy.md)
...
**次: [プロジェクト構造 →](./02-project-structure.md)**
...
#### 5.2.2.2 2️⃣ Application（アプリケーション層）
...
#### 5.2.2.3 3️⃣ Presentation（プレゼンテーション層）
...
#### 5.2.2.4 4️⃣ Infrastructure（インフラ層）
...
**次: [エンティティ設計 →](./03-entity-design.md)**
...
### 5.3.4 📝 実装例 1: Email 値オブジェクト

#### 5.3.4.1 ❌ 悪い例（値オブジェクトがない）
...
#### 5.4.4.1 1️⃣ ユースケース定義（リクエスト/レスポンス）
...
#### 5.4.4.2 2️⃣ インターフェース定義（依存性の抽象化）
...
### 5.4.7 🌐 Controller から ユースケース呼び出し
...
**次: [完全実装例 →](./05-implementation-example.md)**
...
### 5.5.2 💾 01: Domain 層の実装

#### 5.5.2.1 domain/errors/DomainError.ts
...
#### 5.5.2.2 domain/value-objects/Email.ts
...
#### 5.5.2.3 domain/value-objects/Password.ts
...
#### 5.5.2.4 domain/entities/User.ts
...
#### 5.5.2.5 domain/interfaces/IUserRepository.ts
...
### 5.5.3 🔄 02: Application 層の実装

#### 5.5.3.1 application/errors/ApplicationError.ts
...
#### 5.5.3.2 application/dtos/RegisterUserRequest.ts
...
#### 5.5.3.3 application/dtos/LoginUserRequest.ts
...
#### 5.5.3.4 application/interfaces/
...
#### 5.5.3.5 application/usecases/RegisterUserUseCase.ts
...
#### 5.5.3.6 application/usecases/LoginUserUseCase.ts
...
### 5.5.4 🌐 03: Presentation 層の実装

#### 5.5.4.1 presentation/controllers/UserController.ts
...
#### 5.5.4.2 presentation/routes/userRoutes.ts
...
### 5.5.5 💾 04: Infrastructure 層の実装

#### 5.5.5.1 infrastructure/database/MySQLConnection.ts
...
#### 5.5.5.2 infrastructure/repositories/UserRepository.ts
...
#### 5.5.5.3 infrastructure/services/BcryptHasher.ts
...
#### 5.5.5.4 infrastructure/services/EmailAdapter.ts
...
#### 5.5.5.5 infrastructure/services/JwtTokenGenerator.ts
...
### 5.5.6 ⚙️ 05: Config 層（DI 設定）

#### 5.5.6.1 config/Container.ts
...
**次: [テスト戦略 →](./06-testing-strategy.md)**

## 5.6 テスト戦略

クリーンアーキテクチャで各層を効率的かつ高品質にテストする方法を学びます。
...
### 5.6.2 🧪 01: Domain 層テスト（ユニットテスト）

#### 5.6.2.1 Domain テストの特徴

- **外部依存なし** - DB、API 呼び出しなし
- **最も高速** - 数ミリ秒で実行
- **本数が多い** - ビジネスロジックをすべてカバー

#### 5.6.2.2 Email 値オブジェクトテスト
...
### 5.6.3 📋 02: Application 層テスト

#### 5.6.3.1 Application テストの特徴

- **モックが必要** - Repository、ExternalService をモック
- **ユースケース全体をテスト** - エンドツーエンドロジック
- **テストダブル** - Mock / Stub

#### 5.6.3.2 RegisterUserUseCase テスト
...
### 5.6.5 🧪 04: Controller（Presentation）テスト
...
| # | トピック | 内容 |
|---|---------|------|
| [02-naming-conventions.md](./02-naming-conventions.md) | **命名規則** | チーム全体で一貫した命名のルール |
| [03-error-handling.md](./03-error-handling.md) | **エラーハンドリング** | 層別のエラー処理戦略 |
| [04-logging-monitoring.md](./04-logging-monitoring.md) | **ロギング・監視** | 本番環境での可視化 |
| [05-performance-optimization.md](./05-performance-optimization.md) | **パフォーマンス** | N+1問題、キャッシング等 |
| [06-security.md](./06-security.md) | **セキュリティ** | 入力検証、認証・認可、SQL インジェクション対策 |
...
**詳細 → [命名規則へ](./02-naming-conventions.md)**

#### 6.1.2.2 2️⃣ エラーハンドリング

##### 6.1.2.2.1 ドメイン層：ビジネスエラー例外
...
##### 6.1.2.2.3 プレゼンテーション層：HTTP ステータス
...
##### 6.1.2.5.2 Authentication/Authorization
...
**次: [エラーハンドリング →](./03-error-handling.md)**
...
#### 6.3.4.2 パターン2: Result オブジェクトで結果を返す
...
**次: [ロギング・監視 →](./04-logging-monitoring.md)**
...
#### 6.4.3.1 プレゼンテーション層：リクエスト/レスポンス
...
**次: [パフォーマンス最適化 →](./05-performance-optimization.md)**
...
### 6.6.3 🔑 Authentication/Authorization
...
#### 6.6.3.2 Role-Based Access Control（RBAC）
...
- [エラーハンドリング](./03-error-handling.md) - エラーレスポンス
- [ロギング・監視](./04-logging-monitoring.md) - セキュリティイベント監視
...
| # | アンチパターン | 特徴 | 対策 |
|---|-------------|------|------|
| [01-over-engineering.md](./01-over-engineering.md) | **過度な設計** | 必要以上に分層・複雑化 | 段階的な設計 |
| [03-tight-coupling.md](./03-tight-coupling.md) | **密結合** | 層間の依存関係が逆向き | 依存関係の逆転 |
| [04-anemic-model.md](./04-anemic-model.md) | **貧血モデル** | データホルダーのみ | リッチモデル化 |
| [05-circular-dependency.md](./05-circular-dependency.md) | **循環依存** | モジュール間の循環参照 | インターフェース分離 |

---

### 7.2.2 ❌ 1. 過度な設計（Over-Engineering）
...
### 7.2.5 ❌ 4. 循環依存（Circular Dependency）
...
### 7.3.4 🔄 よくある密結合パターンと修正

#### 7.3.4.1 パターン1：グローバルゴブリン
...
**次: [貧血モデルの回避 →](./04-anemic-model.md)**

## 7.4 貧血モデル（Anemic Model）

ビジネスロジックをエンティティに戻す（リッチモデル設計）。
...
// 2. ロジックが一貫していない
//    （TransferUseCase では < だが、別の場所では ≤ かもしれない）
// 3. ビジネスルールがどこにあるか不明
// 4. テストが複雑（モック設定多い）
```
...
以下の兆候が複数当てはまる場合、貧血モデルを疑う：

```
✅ Entity クラスがフィールドと getter/setter しかない
✅ if 文や計算ロジックが Entity ではなく UseCase/Service 側にある
✅ 同じ検証ロジック（残高チェックなど）が複数の UseCase に重複している
✅ Entity に業務的な操作（transfer、withdraw など）を表すメソッドがない
```

**コードレビューでの着眼点：**
- Entity のフィールドが `public` で、外部から自由に書き換えられる
- 「〜Service」が単一の Entity だけを操作し、その Entity 内に置けるはずのロジックを持っている
...
✅ Use Case は層の仲介のみ
✅ Service は domain service（複数エンティティにまたがるロジック）のみ
...
**次: [循環依存の回避 →](./05-circular-dependency.md)**

## 7.5 循環依存（Circular Dependency）
...
#### 7.5.6.3 Webpack/TypeScript での検出
...
#### 7.5.6.4 TypeScript での検出（tsc --diagnostics）
...
- [密結合の回避](./03-tight-coupling.md) - インターフェース設計
...
| # | プロジェクト | スケール | 複雑度 | 重点 |
|---|-----------|--------|------|------|
| [02-ecommerce-site.md](./02-ecommerce-site.md) | **ECサイト** | 中規模 | ★★★★☆ | 複数の決済方法、在庫管理、注文フロー |
| [03-sns-platform.md](./03-sns-platform.md) | **SNS** | 大規模 | ★★★★★ | フィード生成、キャッシング、スケーリング |
| [04-microservices.md](./04-microservices.md) | **マイクロサービス** | 大規模 | ★★★★★ | サービス分割、非同期通信、サガパターン |

---

### 8.1.2 📦 ケース1: ECサイト（商品販売）
...
**次: [SNS プラットフォーム →](./03-sns-platform.md)**

## 8.3 SNS プラットフォーム（フィード生成・キャッシング）
...
ユーザーが投稿を作成・閲覧して、フォロワーのタイムラインにフィードが表示されるSNS。
...
**次: [マイクロサービス →](./04-microservices.md)**

## 8.4 マイクロサービス（サービス分割・イベント駆動）
...
#### 8.4.3.3 Inventory Service：イベントリスナー
...
#### 8.4.3.4 Payment Service：イベントリスナー
...
| # | カテゴリ | 内容 |
|---|---------|---|
| [02-frameworks.md](./02-frameworks.md) | **フレームワーク** | Express, NestJS, Spring Boot, FastAPI 比較 |
| [03-di-containers.md](./03-di-containers.md) | **DI/IoC Container** | Type-DI, InversifyJS, Awilix, Spring DI |
| [04-development-tools.md](./04-development-tools.md) | **開発ツール** | Jest, Supertest, TestContainers, ESLint, madge, Snyk |
| [05-learning-resources.md](./05-learning-resources.md) | **学習リソース** | 書籍、Udemy/Pluralsight、ブログ、GitHub、コミュニティ |

---

### 9.1.2 🔧 推奨フレームワーク

#### 9.1.2.1 Node.js / TypeScript
...
### 9.1.3 💉 DI Container・IoC フレームワーク

#### 9.1.3.1 TypeScript 推奨
...
| **エコシステム** | ライブラリ・ツールの豊富さ |

---

### 9.2.2 🔷 Node.js / TypeScript フレームワーク

#### 9.2.2.1 ① Express.js + Type-DI
...
### 9.2.4 🐍 Python フレームワーク

#### 9.2.4.1 ① FastAPI + Dependency Injector
...
**次: [DI コンテナ →](./03-di-containers.md)**

## 9.3 DI コンテナー・IoC フレームワーク
...
### 9.3.2 🔷 TypeScript 推奨 3選

#### 9.3.2.1 ① Type-DI（最もシンプル）
...
#### 9.3.3.1 ① Spring DI（Spring Boot 組み込み）
...
**次: [開発ツール →](./04-development-tools.md)**

## 9.4 開発ツール（テスト・分析・ビルド）
...
#### 9.4.1.1 ユニットテスト：Jest（Node.js 標準）
...
#### 9.4.1.2 統合テスト：Supertest（HTTP テスト）
...
#### 9.4.1.3 E2E テスト：TestContainers（本物の DB でテスト）
...
#### 9.4.2.1 静的解析：ESLint + TypeScript Plugin
...
**次: [学習リソース →](./05-learning-resources.md)**
...
#### 9.5.1.1 第1順位：「Clean Architecture」 Robert C. Martin
...
#### 9.5.1.2 第2順位：「Domain-Driven Design」Eric Evans
...
#### 9.5.1.3 第3順位：「Building Microservices」Sam Newman
...
#### 9.5.1.4 番外編：「Refactoring」Martin Fowler
...
#### 9.5.4.1 GitHub リポジトリ

##### 9.5.4.1.1 ① clean-architecture-manga
...
##### 9.5.4.1.3 ③ typescript-clean-architecture
